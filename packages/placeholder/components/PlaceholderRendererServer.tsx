...
async function PlaceholderItemServer({
  ...
  if (item.kind === "divider") {
    return <ClientRenderer items={[item]} dividerComponent={dividerComponent} />;
  }
  ...
  if (item.kind === "slot") {
    ...
    return <ClientRenderer items={[slotItem]} dividerComponent={dividerComponent} />;
  }
  ...
  if (item.kind === "component") {
    ...
    const isThunk = (() => {
      ...
      try {
        ...
        return probe != null && typeof (probe as Promise<unknown>).then === "function";
      } catch {
        return false;
      }
    })();
    ...
    if (isThunk) {
      const mod = await (
        component as () => Promise<{ default: React.ComponentType<Record<string, unknown>> }>
      )();
      return <mod.default {...(props as Record<string, unknown>)} />;
    }
